...
#'@param X1LP Lower Strike Price or eXercise price bought Put.
#'@param X2HC Higher Strike Price or eXercise price bought Call.
...
#'@export
aStranglePnL <- function (ST,X1LP,X2HC,C,P,hl=0,hu=2.2,spot=spot,pl=pl,myData=myData,myTibble=myTibble,PnL=PnL){
  V0Dr=C+P
  myData <- data.frame (spot = c((ST*hl):(ST*hu)))
  myData$pl <- (pmax((myData$spot-X2HC),0)+(pmax((X1LP-myData$spot),0))-V0Dr)
  ...
  ggplot(myTbColored,aes(x=spot,y=pl,fill=PnL,label=pl)) +
    ...
    labs(title  =  ' A Long Strangle ', x  =  'Spot Price ($) at Expiration', y  =  'PnL ($) at Expiration', subtitle  =  'High Volatility / Neutral Outlook', caption  =  'volatilityTrader / MaheshP Kumar')
}
